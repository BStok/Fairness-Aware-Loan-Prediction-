...
results_dir = r"C:\Users\projects\Fairness-Aware-Loan-Prediction-\results\baseline\decision_tree"
...
# Pipeline — no scaler needed for DT
pipeline = Pipeline([
...
fig.suptitle("Loan Approval — Decision Tree Diagnostics", fontsize=14, fontweight="bold")
...
plt.tight_layout()
plt.savefig(os.path.join(results_dir, "decision_tree_diagnostics.png"), dpi=150, bbox_inches="tight")
print("\nDiagnostic plot saved → results/baseline/decision_tree_diagnostics.png")
plt.show()
...
pred_df.to_csv(os.path.join(results_dir, "decision_tree_predictions.csv"), index=False)
print("Predictions saved → results/baseline/decision_tree_predictions.csv")
